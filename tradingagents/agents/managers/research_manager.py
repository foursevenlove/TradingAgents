...
def _format_debate_turns(debate_turns: list[dict]) -> str:
    ...
    for turn in debate_turns:
        speaker = turn.get("label") or ("看涨分析师" if turn.get("speaker") == "bull" else "看跌分析师")
...
def _last_turn_content(debate_turns: list[dict], speaker: str, fallback_history: str, prefix: str) -> str:
    """优先从结构化轮次取某方最后发言，兼容旧历史字符串。"""
    for turn in reversed(debate_turns):
        if turn.get("speaker") == speaker:
            label = turn.get("label") or ("看涨分析师" if speaker == "bull" else "看跌分析师")
...
def create_research_manager(llm, memory):
    def research_manager_node(state) -> dict:
        ...
        # 提取各方最后一轮发言，同时保留完整结构化辩论供轮次引用和质量评估
        debate_transcript = _format_debate_turns(debate_turns) if debate_turns else history.strip()
        bull_last = _last_turn_content(debate_turns, "bull", bull_history, "看涨分析师：")
        bear_last = _last_turn_content(debate_turns, "bear", bear_history, "看跌分析师：")
...
3. 必须总结看涨、看跌方各3至5条核心论据，论据必须标注来源于哪位分析师第几轮发言或哪份报告。论据数量应反映辩论实际质量，若某方仅有2条有数据支撑的论据，不得凑数
...
============================================================
【核心决策逻辑（强制执行）】
1. 以【核心数据表】和原始报告中的可验证数据为最高优先级，所有辩论观点都必须接受数据校验
2. 以【完整结构化辩论记录】和【看涨/看跌分析师最后一轮发言】作为候选解释来源，评估双方是否正确引用数据、是否回应对方、是否存在选择性引用
3. 数据冲突处理：当辩论结论与核心数据表出现矛盾时，以核心数据表和原始报告为准；对应辩论论据必须降权、修正或驳回，并明确说明矛盾点
4. 辩论质量评估：必须评估双方辩论的论证质量，包括是否存在逻辑谬误、是否真正回应了对方反驳还是自说自话、数据引用是否准确。辩论质量只影响解释权重，不得覆盖原始数据
...
============================================================
【信息优先级指引】
- 最高优先级：核心数据表和原始报告中的可验证数据
- 高优先级：完整结构化辩论记录与看涨/看跌分析师最后一轮发言（用于提取候选论据和评估辩论质量）
